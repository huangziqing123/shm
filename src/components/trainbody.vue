...
export default {
  ...
  data() {
    return {
      ...
      sensorinfo: [
        { name: "Maximum acc.:", val: this.max },
        { name: "Minimum acc.:", val: this.mini },
        { name: "Main Frequency:", val: this.fre },
        { name: "Spring Index:", val: this.index },
      ],
      max: 0,
      mini: 0,
      fre: 0,
  ...
  mounted: function () {
    ...
    $.ajax({
      url: this.url + "api/v1/sensor/list/?place=0",
      type: "get",
      async: false,
      xhrFields: {
  ...
  methods: {
      ...
      withCredentials: true
            ...
            success: function (ret) {
              for(var i=0;i<ret[0].sensors.length;i++){
                  that.tableData.push({
                      name:"Description of sensor "+ret[0].sensors[i].number,
                      id:ret[0].sensors[i].id
                  })
              }
            }
        })
