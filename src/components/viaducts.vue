...
export default {
    ...
    mounted: function () {
        ...
        $.ajax({
            url:
                "http://10.0.2.20:8000/api/v1/getviaductspageinfo/?piczoom=" +
                this.cxzoom.toString(),
            type: "GET",
            async: false,
            success: function (ret) {
                that.sensordata = ret.sensorinfo;
                that.str = ret.viaductsinfo.url;
                that.dataa = ret.viaductsinfo.viaductsinfo;
                scale = that.width / ret.viaductsinfo.width;
                that.cxzoom = scale;
                for (let i = 0; i < that.dataa.length; i++) {
                    that.dataa[i].value[0] *= scale;
                    that.dataa[i].value[1] *= scale;
                }
                that.cximagew = ret.viaductsinfo.width;
                that.cximageh1 = ret.viaductsinfo.height;
            },
        });
